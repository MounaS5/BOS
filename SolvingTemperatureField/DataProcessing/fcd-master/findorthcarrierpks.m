...
% get k-space coordinates, turn absolute coordinates to vectors
...
%kmax=13 ; kmin = 2;
%g = f_h;
% as I see it it makes sure the peaks are detected. 
%It does it by making a certain specified arear completely zero so that the
%peaks stand out.Don't have to specify if your peaks are tall enough.
...
% its basically telling its self that threshold is half the value of highest
% peak
if(nargin < 4) 
    thresh = .7*max(f_h(:));
else
...
%gives the coordinates of the peaks with sub-pixel accuracy 
...
% carrier peaks could be like some vector coordinates in space
% hence they require x and y component
